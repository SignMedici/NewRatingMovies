<template>
  <div v-if="isAdmin === true">
    <div>
      <nuxt-link :to="{ name: `movie-add___${locale}`, hash: '#logo' }" class="btn btn-success mb-2 me-2">
        {{ $t('add') }}
      </nuxt-link>
      <nuxt-link :to="{ name: `movie-metadata___${locale}`,hash:'#logo'}" class="btn btn-success mb-2">
        {{ $t('modifyMetadata') }}
      </nuxt-link>
    </div>
    <table class="table table-striped borderless">
      ...
      <tbody v-if="getMovies.length > 0">
        <tr v-for="movie in getMovies" :key="movie._id">
            ...
            <td>
              <div class="d-flex">
                <nuxt-link class="me-2" :to="{ name: `admin-movies-update-id___${locale}`, params: { id: movie._id }, hash: '#logo' }">
